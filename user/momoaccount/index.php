...
if (isset($_POST['submit'])) {
    ...
    $sql_momo_check = "SELECT * FROM momoaccount WHERE Username = '".$_SESSION['name_member']."' ";
    $result_momo_check = mysqli_query($conn, $sql_momo_check);
    $count_momo_check = mysqli_num_rows($result_momo_check);
    if ($count_momo_check == 0) {
        //Check number
        $sql_momo_check_num = "SELECT * FROM momoaccount WHERE MomoNumber = '$number' ";
        $result_momo_check_num = mysqli_query($conn, $sql_momo_check_num);
        $count_momo_check_num = mysqli_num_rows($result_momo_check_num);
        if ($count_momo_check_num == 0) {
            $sql_momo = "INSERT INTO momoaccount (Username, MomoCarrier, MomoName, MomoNumber, ModificationDate) VALUES ('$username', '$carrier', '$name', '$number', '$date') ";
            $result_momo = mysqli_query($conn, $sql_momo) or die(mysqli_error($conn));
            if ($result_momo) {
                $msg_success = "Mobile money account details successfully addded";
            }else{
                $msg_error = "Failed to add mobile money account details. Please try again later";
            }
        }else{
            $msg_error = "Mobile money number already exits in the database for another user.";
        }
    }else{
        //Check number
        $sql_momo_check_num = "SELECT * FROM momoaccount WHERE MomoNumber = '$number' ";
        ...
        $count_momo_check_num = mysqli_num_rows($result_momo_check_num);
        if ($count_momo_check_num == 0) {
            $sql_momo = "UPDATE momoaccount SET MomoCarrier = '$carrier', MomoName = '$name', MomoNumber = '$number', ModificationDate = '$date' WHERE Username = '$username' ";
            $result_momo = mysqli_query($conn, $sql_momo) or die(mysqli_error($conn));
            if ($result_momo) {
                $msg_success = "Mobile money account details successfully updated";
            }else{
                $msg_error = "Failed to update mobile money account details. Please try again later";
            }
        }else{
            $row_momo_check_num = mysqli_fetch_assoc($result_momo_check_num);
            if ($username == $row_momo_check_num['Username']) {
                $sql_momo = "UPDATE momoaccount SET MomoCarrier = '$carrier', MomoName = '$name', MomoNumber = '$number', ModificationDate = '$date' WHERE Username = '$username' ";
                $result_momo = mysqli_query($conn, $sql_momo) or die(mysqli_error($conn));
                if ($result_momo) {
                    $msg_success = "Mobile money account details successfully updated";
...
<head>
    ...
    <title>Mobile Money Account - Secure Prime Investment</title>
	<!-- Bootstrap Styles-->
        ...
        <nav class="navbar-default navbar-side" role="navigation">
            <div class="sidebar-collapse">
                <ul class="nav" id="main-menu">
                    ...
                    <li>
                        <a class="active-menu" href="/SecurePrimeInvestment/user/momoaccount/"><i class="fa fa-usd"></i>MOBILE MONEY ACCOUNT</a>
                    </li>
                    <li>
                    ...
                    <li>
                        <a  href="/SecurePrimeInvestment/user/testimony/"><i class="fa fa-share"></i> SHARE TESTIMONY</a>
                    </li>
                    <li>
                        ...
                        <div class="row">
                            <div class="col-md-12 col-lg-12 col-sm-12">
                                ...
                                <div class="card">
                                    ...
                                    $count_momo_check = mysqli_num_rows($result_momo_check);
                                    if ($count_momo_check == 0) { ?>
                                    <div class="card-header"><h3>Add Mobile Money Account</h3></div> <br>
                                    <?php
                                    if (isset($msg_error)) { ?>
                                    ...
                                    <div class="card-body">
                                        <form class="forms-sample" action="index.php" method="POST">
                                            <div class="form-group">
                                                <label for="momoCarrier">Mobile Money Carrier</label>
                                                <select class="form-control" id="momoCarrier" name="carrier" required>
                                                    <option value="" selected disabled hidden>Select Mobile Money Carrier</option>
                                                    <option value="MTN">MTN Cameroon</option>
                                                    <option value="Orange">Orange Cameroon</option>
                                                </select>
                                            </div>
                                            <div class="form-group">
                                                <label for="momoName">Mobile Money Name</label>
                                                <input type="text" class="form-control" id="momoName" placeholder="Mobile Money Name" name="name" required>
                                            </div>
                                            <div class="form-group">
                                                <label for="momoNumber">Mobile Money Number</label>
                                                <input type="tel" class="form-control" id="momoNumber" placeholder="Mobile Money Number" name="number" pattern="[6]{1}[0-9]{8}" title="Phone number should be of 9 digits and must begin with a 6" required>
                                            </div>
                                            <div class="form-group">
                                                <button type="submit" name="submit" class="form-control bg-blue btn btn-primary"><i class="ik ik-plus-circle"></i> ADD</button>
                                            </div>
                                        </form>
                                    </div>
                                    <?php }else{
                                        $row_momo_check = mysqli_fetch_assoc($result_momo_check);
                                        ?>
                                    <div class="card-header"><h3>Edit Mobile Money Account</h3></div> <br>
                                    <?php
                                    if (isset($msg_error)) { ?>
                                        <div class="alert alert-danger" role="alert">
                                            <?php echo "" . $msg_error . ""; ?>
                                    ...
                                    <div class="card-body">
                                        <form class="forms-sample" action="index.php" method="POST">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" id="momoCarrier" name="carrier" required>
                                                    <?php
                                                    if ($row_momo_check['MomoCarrier'] == 'MTN') { ?>
                                                        <option value="MTN" selected>MTN Cameroon</option>
                                                        <option value="Orange">Orange Cameroon</option>
                                                    <?php }elseif ($row_momo_check['MomoCarrier'] == 'Orange') { ?>
                                                        <option value="Orange" selected>Orange Cameroon</option>
                                                        <option value="MTN">MTN Cameroon</option>
                                                    <?php } ?>
                                                </select>
                                            </div>
                                            <div class="form-group">
                                                <label for="momoName">Mobile Money Name</label>
                                                <input type="text" class="form-control" id="momoName" placeholder="Mobile Money Name" name="name" required value="<?php echo "" . $row_momo_check['MomoName'] . ""; ?>">
                                            </div>
                                            <div class="form-group">
                                                <label for="momoNumber">Mobile Money Number</label>
                                                <input type="tel" class="form-control" id="momoNumber" placeholder="Mobile Money Number" name="number" pattern="[6]{1}[0-9]{8}" title="Phone number should be of 9 digits and must begin with a 6" required value="<?php echo "" . $row_momo_check['MomoNumber'] . ""; ?>">
                                            </div>
                                            <div class="form-group">
                                                <button type="submit" name="submit" class="form-control bg-blue btn btn-primary"><i class="ik ik-edit"></i> UPDATE</button>
                                            </div>
                                        </form>
                                    </div>
                                    <?php }
