...
<head>
    ...
    <title>List of Pledge Provided - Secure Prime Investment</title>
	<!-- Bootstrap Styles-->
        ...
        <nav class="navbar-default navbar-side" role="navigation">
            <div class="sidebar-collapse">
                <ul class="nav" id="main-menu">
                    ...
                    <li>
                        <a  class="active-menu" href="/SecurePrimeInvestment/user/pledgeprovided/"><i class="fa fa-list"></i> LIST OF PLEDGE PAID</a>
                    </li>
                    <li>
                                    ...
                                    <div class="card-body p-0 table-border-style">
                                        <div class="dataTable_wrapper table-responsive">
                                            <table border="0" width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                                ...
                                                <tbody>
                                                    <?php
                                                    $sql_help_provided = "SELECT * FROM matching WHERE Status = 'Paid' AND PledgeNo IN (SELECT SN FROM helprequest WHERE Matched = '1' AND Username = '".$_SESSION['name_member']."' ) ORDER BY ConfirmationDate DESC ";
                                                    $result_help_provided = mysqli_query($conn, $sql_help_provided);
                                                    while ($row_help_provided = mysqli_fetch_assoc($result_help_provided)) {
                                                        $sql_plege_amt = "SELECT * FROM helprequest WHERE SN = '".$row_help_provided['PledgeNo']."' ";
                                                        $result_plege_amt = mysqli_query($conn, $sql_plege_amt);
                                                        $row_plege_amt = mysqli_fetch_assoc($result_plege_amt);
                                                    ?>
                                                    <tr>
                                                        <th style="height: 30px;" scope="row"><?php echo "" . $row_help_provided['SN'] . ""; ?></th>
                                                        <th style="height: 30px;" scope="row"><?php echo "" . $row_help_provided['PledgeNo'] . ""; ?></th>
                                                        <th style="height: 30px;" scope="row"><?php echo "CFA " . number_format($row_plege_amt['Amount'], 2, '.', ',') . ""; ?></th>
                                                        <th style="height: 30px;" scope="row"><?php echo "" . $row_help_provided['MatchTo'] . ""; ?></th>
                                                        <th style="height: 30px;" scope="row">
                                                            <img src="../<?php echo "" . $row_help_provided['Proof'] . ""; ?>" height="30px;" width="30px;">
                                                        </th>
                                                        <?php
                                                        if (empty($row_help_provided['ConfirmationDate'])) { ?>
                                                            <th style="height: 30px;" scope="row"><span style="color: red;"><strong>Pending confirmation...</strong></span></th>
                                                        <?php }else { ?>
                                                            <th style="height: 30px;" scope="row"><span style="color: blue;"><strong>Confirmed...</strong></span></th>
                                                        <?php } ?>
                                                    </tr>
                                                    <?php } ?>
                                                </tbody>
