...
<head>
    ...
    <title>List of Pledge To Receive - Secure Prime Investment</title>
	<!-- Bootstrap Styles-->
        ...
        <nav class="navbar-default navbar-side" role="navigation">
            <div class="sidebar-collapse">
                <ul class="nav" id="main-menu">
                    ...
                    <li>
                        <a  class="active-menu" href="/SecurePrimeInvestment/user/pledgetoreceive/"><i class="fa fa-list"></i> LIST OF PLEDGE TO RECEIVE</a>
                    </li>
                     <li>
                                    ...
                                    <div class="card-body p-0 table-border-style">
                                        <div class="dataTable_wrapper table-responsive">
                                            <table border="0" width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                                ...
                                                <tbody>
                                                    <?php
                                                    $sql_help_received = "SELECT * FROM helptoreceive WHERE Balance != '0' AND PledgeNo IN (SELECT SN FROM helprequest WHERE Username = '".$_SESSION['name_member']."') ORDER BY LastDateToReceive DESC ";
                                                    $result_help_received = mysqli_query($conn, $sql_help_received);
                                                    while ($row_help_received = mysqli_fetch_assoc($result_help_received)) {
                                                        $sql_help_request = "SELECT * FROM helprequest WHERE SN = '".$row_help_received['PledgeNo']."' ";
                                                        $result_help_request = mysqli_query($conn, $sql_help_request);
                                                        $row_help_request = mysqli_fetch_assoc($result_help_request);
                                                    ?>
                                                    <tr>
                                                        <th style="height: 30px;" scope="row"><?php echo "" . $row_help_received['SN'] . ""; ?></th>
                                                        <th style="height: 30px;" scope="row"><?php echo "" . $row_help_received['PledgeNo'] . ""; ?></th>
                                                        <th style="height: 30px;" scope="row"><?php echo "CFA " . number_format($row_help_request['Amount'], 2, '.', ',') . ""; ?></th>
                                                        <th style="height: 30px;" scope="row"><?php echo "CFA " . number_format($row_help_received['AmountToReceive'], 2, '.', ',') . ""; ?></th>
                                                        <th style="height: 30px;" scope="row"><?php echo "CFA " . number_format($row_help_received['AmountReceived'], 2, '.', ',') . ""; ?></th>
                                                        <th style="height: 30px;" scope="row"><?php echo "CFA " . number_format($row_help_received['Balance'], 2, '.', ',') . ""; ?></th>
                                                        <?php
                                                        $date = date("Y-m-d H:i:s");
                                                        $lastdate00 = strtotime($row_help_received['LastDateToReceive']);
                                                        $remaining00 = $lastdate00 - time();
                                                        $days_remaining00 = floor($remaining00 / 86400);
                                                        $hours_remaining00 = floor(($remaining00 % 86400) / 3600);
                                                        $minutes_remaining00 = floor(($remaining00 % 3600) / 60);
                                                        $seconds_remaining00 = ($remaining00 % 60);
                                                        if ($date > $row_help_received['LastDateToReceive']) { ?>
                                                            <th style="height: 30px;" scope="row"><span style="color: red;"><strong><?php echo "" . $row_help_received['LastDateToReceive'] . ""; ?></strong></span></th>
                                                            <th style="height: 30px;" scope="row"><span style="color: red;"><strong>Dued for payment</strong></span></th>
                                                            <th style="height: 30px;" scope="row"><span style="color: red; font-weight: bolder;" id="countdown00" class="timer"></span></th>
                                                        <?php }else { ?>
                                                            <th style="height: 30px;" scope="row"><span style="color: blue;"><strong><?php echo "" . $row_help_received['LastDateToReceive'] . ""; ?></strong></span></th>
                                                            <th style="height: 30px;" scope="row"><span style="color: blue;"><strong>Not due for payment</strong></span></th>
                                                            <th style="height: 30px;" scope="row"><span style="color: blue; font-weight: bolder;" id="countdown00" class="timer"></span></th>
                                                        <?php } ?>
                                                    </tr>
                                                    <?php } ?>
                                                </tbody>
