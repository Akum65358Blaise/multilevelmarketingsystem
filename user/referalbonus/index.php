...
<head>
    ...
    <title>Referal Bonus - Secure Prime Investment</title>
	<!-- Bootstrap Styles-->
        ...
        <nav class="navbar-default navbar-side" role="navigation">
            <div class="sidebar-collapse">
                <ul class="nav" id="main-menu">
                    ...
                    <li>
                        <a  class="active-menu" href="/SecurePrimeInvestment/user/referalbonus/"><i class="fa fa-plus-circle"></i> REFERAL BONUS</a>
                    </li>
                    <li>
                                    ...
                                    <div class="card-body p-0 table-border-style">
                                        <div class="dataTable_wrapper table-responsive">
                                            <table border="0" width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                                ...
                                                <tbody>
                                                    <?php
                                                    $sql_referal_bonus = "SELECT * FROM referalbonus WHERE Username = '".$_SESSION['name_member']."' ORDER BY PaymentDate DESC ";
                                                    $result_referal_bonus = mysqli_query($conn, $sql_referal_bonus);
                                                    while ($row_referal_bonus = mysqli_fetch_assoc($result_referal_bonus)) {
                                                        $sql_help_request = "SELECT * FROM helprequest WHERE SN = '".$row_referal_bonus['PledgeNo']."' ";
                                                        $result_help_request = mysqli_query($conn, $sql_help_request);
                                                        $row_help_request = mysqli_fetch_assoc($result_help_request);
                                                    ?>
                                                    <tr>
                                                        <th style="height: 30px;" scope="row"><?php echo "" . $row_referal_bonus['SN'] . ""; ?></th>
                                                        <th style="height: 30px;" scope="row"><?php echo "" . $row_referal_bonus['PledgeNo'] . ""; ?></th>
                                                        <th style="height: 30px;" scope="row"><?php echo "" . $row_help_request['Username'] . ""; ?></th>
                                                        <th style="height: 30px;" scope="row"><?php echo "CFA " . number_format($row_help_request['Amount'], 2, '.', ',') . ""; ?></th>
                                                        <th style="height: 30px;" scope="row"><?php echo "CFA " . number_format($row_referal_bonus['Amount'], 2, '.', ',') . ""; ?></th>
                                                        <th style="height: 30px;" scope="row"><?php echo "" . $row_referal_bonus['Status'] . ""; ?></th>
                                                        <?php
                                                        if (empty($row_referal_bonus['PaymentDate'])) { ?>
                                                            <th style="height: 30px;" scope="row"><span style="color: red;"><strong>Pending approval...</strong></span></th>
                                                        <?php }else { ?>
                                                            <th style="height: 30px;" scope="row"><span style="color: blue;"><strong><?php echo "" . $row_referal_bonus['PaymentDate'] . ""; ?></strong></span></th>
                                                        <?php } ?>
                                                    </tr>
                                                    <?php } ?>
                                                </tbody>
