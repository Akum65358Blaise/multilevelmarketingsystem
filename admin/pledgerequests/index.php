...
<html>
<head>
	<title>Pledge Requests - Secure Prime Investment</title>
                        	...
                        	<div class="dataTable_wrapper table-responsive">
                                <table border="0" width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <tbody>
                                        <?php
                                        $i = 1;
                                        $sql = "SELECT * FROM helprequest ORDER BY PledgeDate DESC";
                                        $result = mysqli_query($conn, $sql);
                                        while ($row = mysqli_fetch_assoc($result)) {
                                        ?>
                                        <tr>
                                            <td><?php echo "" . $i . ""; ?> <?php $i = $i+1; ?></td>
                                            <td><?php echo "" . $row['Username'] . ""; ?></td>
                                            <td><?php echo "CFA " . number_format($row['Amount'], 2, '.', ',') . ""; ?></td>
                                            <td><?php echo "" . $row['PledgeDate'] . ""; ?></td>
                                            <?php
                                            if ($row['Matched'] == 0 || empty($row['Matched'])) { ?>
                                                <td style="color: red;">Pending</td>
                                            <?php }elseif ($row['Matched'] == 1) { ?>
                                                <td style="color: blue;">Matched</td>
                                            <?php } ?>
                                            <?php
                                            if ($row['Matched'] == 0 || empty($row['Matched'])) { ?>
                                            <td>
                                                <button type="button" class="btn btn-success" data-toggle="modal" data-target="#matchModal<?php echo "" . $row['SN'] . ""; ?>">Match</button>
                                                <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#deleteModal<?php echo "" . $row['SN'] . ""; ?>">Delete</button>
                                            </td>
                                            <?php }elseif ($row['Matched'] == 1) { ?>
                                            <td>
                                            <?php
                                            if ($row['Amount'] == 0) { ?>
                                                <button type="button" disabled class="btn btn-warning" data-toggle="modal" data-target="#unmatchModal<?php echo "" . $row['SN'] . ""; ?>">Unmatch</button>
                                                <button type="button" disabled class="btn btn-danger" data-toggle="modal" data-target="#deleteModal<?php echo "" . $row['SN'] . ""; ?>">Delete</button>
                                            <?php }else{ 
                                            $sql_chk = "SELECT * FROM matching WHERE PledgeNo = '".$row['SN']."' ";
                                            $result_chk = mysqli_query($conn, $sql_chk);
                                            $row_chk = mysqli_fetch_assoc($result_chk);
                                            if (empty($row_chk['ConfirmationDate'])) { ?>
                                                <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#unmatchModal<?php echo "" . $row['SN'] . ""; ?>">Unmatch</button>
                                                <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#deleteModal<?php echo "" . $row['SN'] . ""; ?>">Delete</button>
                                            <?php }else{ ?>
                                                <button type="button" disabled class="btn btn-warning" data-toggle="modal" data-target="#unmatchModal<?php echo "" . $row['SN'] . ""; ?>">Unmatch</button>
                                                <button type="button" disabled class="btn btn-danger" data-toggle="modal" data-target="#deleteModal<?php echo "" . $row['SN'] . ""; ?>">Delete</button>
                                            ...
                                            <?php } ?>
                                                    ...
                                                    <div class="modal fade" id="matchModal<?php echo "" . $row['SN'] . ""; ?>" tabindex="-1" role="dialog" aria-labelledby="matchModalLabel" aria-hidden="true">
                                                        <div class="modal-dialog" role="document">
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title" id="demoModalLabel">Match help request</h5>
                                                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                                                </div>
                                                                <div class="modal-body">
                                                                    <div class="alert alert-success">
                                                                        Click on any of the possible members below to match <strong><?php echo "" . $row['Username'] . ""; ?></strong> to pay him/her <strong><?php echo "CFA " . number_format($row['Amount'], 2, '.', ',') . ""; ?></strong> <br>
                                                                        <ul>
                                                                            <li><a href="matchtome.php?pledgeno=<?php echo "" . $row['SN'] . ""; ?>&amount0=<?php echo "" . $row['Amount'] . ""; ?>"><strong>Match to me</strong></a></li>
                                                                            <?php
                                                                            $date = date("Y-m-d H:i:s");
                                                                            $sql_poss = "SELECT * FROM helptoreceive WHERE Balance != '0' AND Balance >= '".$row['Amount']."' AND Status = 'Available' AND LastDateToReceive < '$date' ORDER BY LastDateToReceive DESC ";
                                                                            $result_poss = mysqli_query($conn, $sql_poss);
                                                                            while ($row_poss = mysqli_fetch_assoc($result_poss)) {
                                                                                $sql_name = "SELECT * FROM helprequest WHERE SN = '".$row_poss['PledgeNo']."' ";
                                                                                $result_name = mysqli_query($conn, $sql_name);
                                                                                $row_name = mysqli_fetch_assoc($result_name);
                                                                            ?>
                                                                            <li><a href="match.php?matchto=<?php echo "" . $row_name['Username'] . ""; ?>&pledgeno=<?php echo "" . $row['SN'] . ""; ?>&pledgeno0=<?php echo "" . $row_name['SN'] . ""; ?>&id=<?php echo "" . $row_poss['SN'] . ""; ?>&amount0=<?php echo "" . $row['Amount'] . ""; ?>"><strong><?php echo "" . $row_name['Username'] . ""; ?></strong> to receive <strong><?php echo "CFA " . number_format($row_poss['Balance'], 2, '.', ',') . ""; ?> </strong> on or before <strong><?php echo "" . date("l F d Y H:i:s", strtotime($row_poss['LastDateToReceive'])) . ""; ?></strong></a></li>
                                                                            <?php } ?>
                                                                        </ul>
                                                                    </div>
                                                                </div>
                                                    ...
                                                    <div class="modal fade" id="unmatchModal<?php echo "" . $row['SN'] . ""; ?>" tabindex="-1" role="dialog" aria-labelledby="unmatchModalLabel" aria-hidden="true">
                                                        <div class="modal-dialog" role="document">
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title" id="demoModalLabel">Unmatch help request</h5>
                                                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                                                </div>
                                                                <div class="modal-body">
                                                                    <div class="alert alert-warning">
                                                                        Are you sure you want to unmatch this help request: <strong><?php echo "CFA " . number_format($row['Amount'], 2, '.', ',') . ""; ?> by <?php echo "" . $row['Username'] . ""; ?>?</strong>
                                                                    </div>
                                                                </div>
                                                                <div class="modal-footer">
                                                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">NO</button>
                                                                    <a href="unmatch.php?id=<?php echo "" . $row['SN'] . ""; ?>&amount0=<?php echo "" . $row['Amount'] . ""; ?>"><button type="button" class="btn btn-primary">YES</button></a>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>

                                                    <div class="modal fade" id="deleteModal<?php echo "" . $row['SN'] . ""; ?>" tabindex="-1" role="dialog" aria-labelledby="deleteModalLabel" aria-hidden="true">
                                                        <div class="modal-dialog" role="document">
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title" id="demoModalLabel">Delete help request</h5>
                                                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                                                </div>
                                                                <div class="modal-body">
                                                                    <div class="alert alert-danger">
                                                                        Are you sure you want to delete this help request: <strong><?php echo "CFA " . number_format($row['Amount'], 2, '.', ',') . ""; ?> by <?php echo "" . $row['Username'] . ""; ?>?</strong>
                                                                    </div>
                                                                </div>
                                                                <div class="modal-footer">
                                                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">NO</button>
                                                                    <a href="delete.php?id=<?php echo "" . $row['SN'] . ""; ?>"><button type="button" class="btn btn-primary">YES</button></a>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
