...
<html>
<head>
	<title>Pledge To Receive - Secure Prime Investment</title>
                        	...
                        	<div class="dataTable_wrapper table-responsive">
                                <table border="0" width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <tbody>
                                        <?php
                                        $i = 1;
                                        $sql = "SELECT * FROM helptoreceive WHERE Balance != '0' AND PledgeNo IN (SELECT SN FROM helprequest) ORDER BY LastDateToReceive DESC";
                                        $result = mysqli_query($conn, $sql);
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            $sql_help_request = "SELECT * FROM helprequest WHERE SN = '".$row['PledgeNo']."' ";
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo "" . $row_help_request['Username'] . ""; ?></td>
                                            <td><?php echo "CFA " . number_format($row_help_request['Amount'], 2, '.', ',') . ""; ?></td>
                                            <td><?php echo "CFA " . number_format($row['AmountToReceive'], 2, '.', ',') . ""; ?></td>
                                            <td><?php echo "CFA " . number_format($row['AmountReceived'], 2, '.', ',') . ""; ?></td>
                                            <td><?php echo "CFA " . number_format($row['Balance'], 2, '.', ',') . ""; ?></td>
                                            <?php
                                            $date = date("Y-m-d H:i:s");
                                            $lastdate00 = strtotime($row['LastDateToReceive']);
                                            ...
                                            if ($date > $row['LastDateToReceive']) { ?>
                                                <th style="height: 30px;" scope="row"><span style="color: red;"><strong><?php echo "" . $row['LastDateToReceive'] . ""; ?></strong></span></th>
                                            ...
                                            <?php }else { ?>
                                                <th style="height: 30px;" scope="row"><span style="color: blue;"><strong><?php echo "" . $row['LastDateToReceive'] . ""; ?></strong></span></th>
