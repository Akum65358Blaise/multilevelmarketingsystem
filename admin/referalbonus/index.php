                        	...
                        	<div class="dataTable_wrapper table-responsive">
                                <table border="0" width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <tbody>
                                        <?php
                                        $i = 1;
                                        $sql = "SELECT * FROM referalbonus ";
                                        $result = mysqli_query($conn, $sql);
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            $sql1 = "SELECT * FROM helprequest WHERE SN = '".$row['PledgeNo']."' ";
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo "CFA " . number_format($row1['Amount'], 2, '.', ',') . ""; ?></td>
                                            <td><?php echo "" . $row['Username'] . ""; ?></td>
                                            <td><?php echo "CFA " . number_format($row['Amount'], 2, '.', ',') . ""; ?></td>
                                            <?php
                                            if ($row['Status'] == 'Paid') { ?>
                                                <td style="color: blue;">Paid</td>
                                                <td style="color: blue;"><?php echo "" . $row['PaymentDate'] . ""; ?></td>
                                                <td>
                                                    <button type="button" disabled class="btn btn-warning" data-toggle="modal" data-target="#paidModal<?php echo "" . $row['SN'] . ""; ?>">Paid</button>
                                                </td>
                                                <?php }elseif ($row['Status'] == 'Not Paid') { ?>
                                                <td style="color: red;">Not Paid</td>
                                                <td style="color: red;">Pending payment</td>
                                                <td>
                                                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#paidModal<?php echo "" . $row['SN'] . ""; ?>">Make Payment</button>
                                                </td>
                                            <?php } ?>
                                        </tr>

                                                    <div class="modal fade" id="paidModal<?php echo "" . $row['SN'] . ""; ?>" tabindex="-1" role="dialog" aria-labelledby="paidModalLabel" aria-hidden="true">
                                                        <div class="modal-dialog" role="document">
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title" id="demoModalLabel">Confirm payment of referal bonus</h5>
                                                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                                                </div>
                                                                <div class="modal-body">
                                                                    <div class="alert alert-danger">
                                                                        Are you sure you want to mark this referal bonus as paid: <strong><?php echo "CFA " . number_format($row['Amount'], 2, '.', ',') . ""; ?>?</strong>
                                                                    </div>
                                                                </div>
                                                                <div class="modal-footer">
                                                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">NO</button>
                                                                    <a href="paid.php?id=<?php echo "" . $row['SN'] . ""; ?>"><button type="button" class="btn btn-primary">YES</button></a>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
